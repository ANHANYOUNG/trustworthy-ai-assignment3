...
MODEL_PATH = MODEL_DIR / "mnist_wide_mlp.onnx"
SAMPLE_PATH = ROOT / "data" / "mnist_sample.npz"   # reuse same sample as mnist_model.py
METADATA_PATH = MODEL_DIR / "mnist_wide_model_metadata.json"

INPUT_NAME = "mnist_wide_input"
OUTPUT_NAME = "mnist_wide_logits"
...
# layer 1: 128 hidden units, ReLU activation  (4x wider than mnist_model.py)
...
def main() -> None:
    ...
    print("[INFO] Training wide MNIST MLP (784->128->10)...")
    ...
    raw_W0 = classifier.coefs_[0] / 255.0  # (784, 128)
    raw_b0 = classifier.intercepts_[0]      # (128,)
    ...
    # Use the same sample index as mnist_model.py for a direct comparison
    sample_data = np.load(SAMPLE_PATH)
    sample_idx = int(sample_data["sample_index"])
    sample_raw = X_test[sample_idx].astype(np.float32)
    sample_label = int(y_test[sample_idx])
    sample_predicted = int(predictions_test[sample_idx])

    metadata = {
        "model": "784-128-10 ReLU MLP (wide)",
